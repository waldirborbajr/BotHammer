...
- [x] **Classificador Naive Bayes (Multinomial) de spam**, complementar à detecção por palavra-chave, com dataset treinável em runtime (sem recompilar).
  Implementado: `moderation::bayes` (`vectorizer.rs` — bag-of-words próprio; `dataset.rs` — corpus semente pt/en/es embutido no binário; `classifier.rs` — `SpamClassifier` sobre `linfa`/`linfa-bayes`, os pacotes nativos do ecossistema Rust ML). O dataset de treino é `bayes::seed_examples()` (semente embutida) + `bayes_training_examples` (SQLite) — tabela alimentada pelos comandos de admin `/trainspam`/`/trainham`, usados em *reply* a uma mensagem real do grupo (`telegram::commands::train`, `AppState::add_training_example`). `AppState::train_bayes` treina o modelo no boot, em todo `/reload` (que também recarrega `alpha`/`max_features`/`threshold`/`min_tokens`/`enabled` de `[bayes]` em `moderation.toml`) e a cada exemplo novo ensinado — sem precisar reiniciar o processo. `engine::analyze_message` consulta `state.bayes.read().await.spam_probability(...)` como sinal extra, em paralelo a `spam::is_spam`.
  _Esforço: médio · Impacto: médio_
  → **Assumido:** o dataset semente é um ponto de partida pequeno, escrito à mão — não há dado real de nenhum grupo; a ideia é que `/trainspam`/`/trainham` o complementem com o uso real do bot. `threshold = 0.90` por padrão continua conservador, já que o dataset começa pequeno. O classificador é único para o bot inteiro (não por chat) — exemplos ensinados num grupo beneficiam todos os grupos que o bot modera.
  → **Não implementado ainda (próximo passo natural):** comando pra listar/remover exemplos de treino já ensinados (hoje só dá pra adicionar, não auditar/desfazer um `/trainspam` equivocado direto pelo Telegram — precisaria mexer direto no SQLite); persistir o modelo treinado (`linfa-bayes` tem feature `serde`) para não precisar retreinar do zero a cada `/reload` quando o dataset crescer muito (irrelevante na escala atual — treino é praticamente instantâneo). `/stats` já mostra quantos exemplos foram ensinados via `sqlite::count_training_examples`.
